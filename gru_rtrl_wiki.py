...
class GRU_RTRL:
    ...
    def __init__(self, 
        ...
        self.num_params = 3*self.hidden_size*(self.embedding_dim+self.hidden_size)
    ...
    def initialize_jacob_rtrl(self):
        ...
        self.J = SparseMatrix(m=self.hidden_size, n=self.num_params)
        ...
        print('Jacobian params: ', self.J.shape[0] * self.J.shape[1])
    ...
    def gru(self, params, x, h):
        ...
        wr_dense = self.Wr.toDense_rtrl(params[self.Wr.start:self.Wr.end,])
        wu_dense = self.Wu.toDense_rtrl(params[self.Wu.start:self.Wu.end,])
        wh_dense = self.Wh.toDense_rtrl(params[self.Wh.start:self.Wh.end,])
        rr_dense = self.Rr.toDense_rtrl(params[self.Rr.start:self.Rr.end,])
        ru_dense = self.Ru.toDense_rtrl(params[self.Ru.start:self.Ru.end,])
        rh_dense = self.Rh.toDense_rtrl(params[self.Rh.start:self.Rh.end,])
    ...
    def forward_step(self, params, x, h, Jh_data):
        ...
        h_Jh = np.dot(grad_h_h, self.J.toDense_rtrl(Jh_data))
        Jh = grad_h_params + h_Jh
    ...
    def calculate_loss(self, params, h, y):
        ...
        logits = np.dot(self.V.toDense_rtrl(params), h)  # Compute raw logits
    ...
    def combineGradients(self, grad_h, grad_out_params, Jh_data):
        grad_rec_params = np.dot(grad_h, self.J.toDense_rtrl(Jh_data))
    ...
    def calculate_grads_step(self, params, x, y, h, Jh_data):
        ...
        #print("hiiiiiiiii", Jh_data.shape) #(hidden_size * self.J.shape[0] * self.J.shape[1], ) -> (hidden_size, self.J.shape[0] * self.J.shape[1])
    ...
    def update_online(self, params, x, y):
        ...
        Jh_data = np.zeros((self.batch_size, self.J.shape[0] * self.J.shape[1]))
    ...
    def run(self, epochs, data, validation_data):
        ...
        for i, k in zip(range(epochs), random.split(self.key, epochs)):
            ...
            self.paramsData, loss = self.update_online(self.paramsData, x, y)
            losses.append(loss)
